...
let token="", period=null, adminToken="", installPrompt=null;
...
async function entry(){try{const j=await api("entryLogin",{password:$("#entryPw").value});token=j.token;$("#loginScreen").classList.add("hidden");$("#app").classList.remove("hidden");await boot()}catch(e){$("#entryMsg").textContent=e.message}}
...
function usernameFromJsonObject(obj){
  const found=[];
  const walk=(v,key="")=>{
    if(v==null)return;
    ...
    if(typeof v==="object"){
      Object.entries(v).forEach(([k,val])=>{
        ...
        if((kl==="username"||kl.includes("user_name")||kl.includes("instagram_username")) && typeof val==="string"){
          const id=norm(val); if(validUsername(id)) found.push(id);
        }
        walk(val,k);
      });
      return;
...
function usernameFromHtml(text){
  const found=[];
  let m;
  const patterns=[
    /(?:username|user name|instagram username)[^A-Za-z0-9._@]{0,80}@?([A-Za-z0-9._]{2,30})/gi,
    /instagram\.com\/([A-Za-z0-9._]{2,30})\/?/gi
  ];
  for(const re of patterns){while((m=re.exec(text)))found.push(m[1])}
  return unique(found);
}
async function detectOwnUsername(zip, excludedPaths=[]){
  const paths=Object.keys(zip.files).filter(p=>!zip.files[p].dir && !excludedPaths.includes(p));
  const strong=paths.filter(p=>/(profile_information|personal_information|account_information|account_details|signup_information|profile_changes)/i.test(p));
  const candidates=[...strong, ...paths.filter(p=>/\.(json|html)$/i.test(p) && !/(followers?|following|close_friends|blocked|pending|recently_unfollowed)/i.test(p)).slice(0,40)];
  const score=new Map();
  for(const path of candidates.slice(0,60)){
    try{
      const text=await zip.files[path].async("string");
      let ids=[];
      if(/\.json$/i.test(path)){
        try{ids=usernameFromJsonObject(JSON.parse(text))}catch(_e){}
      }else{
        ids=usernameFromHtml(text);
      }
      ids.forEach(id=>score.set(id,(score.get(id)||0)+(strong.includes(path)?5:1)));
    }catch(_e){}
  }
...
async function parseZip(file){
 if(!file)throw Error("ZIP 파일을 선택해주세요.");
 const J=await loadJsZip();
 const zip=await J.loadAsync(file);
 const p=findFiles(zip);
 if(!p.followers.length)throw Error("followers_1 파일을 찾지 못했습니다.");
 if(!p.following)throw Error("following 파일을 찾지 못했습니다.");
 let followers=[];
 for(const path of p.followers){
   const text=await zip.files[path].async("string");
   followers.push(...(path.endsWith(".json")?extractJson(text):extractHtml(text)));
 }
 const ft=await zip.files[p.following].async("string");
 const following=p.following.endsWith(".json")?extractJson(ft):extractHtml(ft);
 const ownUsername=await detectOwnUsername(zip,[...p.followers,p.following]);
 return{followers:unique(followers),following:unique(following),ownUsername};
}
function classify(followers,following,members,ownUsername=""){
 const fs=new Set(followers),gs=new Set(following),own=norm(ownUsername);
 const base=members.filter(p=>!own || norm(p.id)!==own);
 const all=base.map(p=>({...p,status:fs.has(p.id)&&gs.has(p.id)?"mutual":!fs.has(p.id)&&gs.has(p.id)?"onlyMe":fs.has(p.id)&&!gs.has(p.id)?"fansOnly":"neither"}));
...
async function analyze(){if(!period?.open)return toast("현재 맞팔확인 기간이 아닙니다.");const b=$("#analyzeBtn");try{b.disabled=true;b.textContent="분석 중...";$("#analysisStatus").textContent="맞팔리스트와 ZIP 파일을 비교하고 있어요.";const [parsed,base]=await Promise.all([parseZip($("#zipInput").files[0]),api("analysisMembers",{token})]);classify(parsed.followers,parsed.following,base.members,parsed.ownUsername);$("#summarySection").classList.remove("hidden");$("#analysisStatus").textContent=parsed.ownUsername?`분석 완료 · 내 계정 @${parsed.ownUsername} 제외 · ${result.all.length}명 기준`:`분석 완료 · ${result.all.length}명 기준`;renderSummary();showTab("all");$("#analysisCard").classList.add("analysis-done-compact");setTimeout(()=>$("#summarySection").scrollIntoView({behavior:"smooth",block:"start"}),80);toast("맞팔분석 완료")}catch(e){$("#analysisStatus").textContent="오류: "+e.message;toast("분석 실패")}finally{b.disabled=!period?.open;b.textContent="맞팔 분석 시작"}}
...
$("#moreBtn").onclick=()=>$("#moreMenu").classList.remove("hidden");$("#logoutBtn").onclick=()=>{token="";location.reload()};$$("[data-close]").forEach(b=>b.onclick=()=>$("#"+b.dataset.close).classList.add("hidden"));
